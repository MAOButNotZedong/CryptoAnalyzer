package ru.javarush.martyshin.cryptoanalyzer.resources;

public class StringValues {
    public static final String PROGRAM_NAME = "Программа CryptoAnalyzer. Зашифровывает и расшифровывает содержимое .txt файлов.";

    public static final String ENCODE_MENU_ITEM = "Закодировать файл.";
    public static final String DECODE_MENU_ITEM = "Декодировать файл с помощью ключа.";
    public static final String BRUTE_FORCE_MENU_ITEM = "Взлом в лоб (Brute Force).";
    public static final String CLOSE_PROGRAM = "Выйти из программы.";
    public static final String CHOOSE_MENU_ITEM_NUM = "Выберите номер пункта меню.";
    public static final String MENU_NUM_NOT_EXIST = "Пункта №%d меню не существует!";
    public static final String CHOOSE_FILE_FOR = "Выберите номер файла для %s из списка.";
    public static final String FILES_NOT_FOUND = "Файлы не найдены. Повторите попытку";
    public static final String ENTER_KEY = "И введите ключ через пробел. Например, 1 15";
    public static final String FOR_ENCODE = "зашифровки";
    public static final String FOR_DECODE = "расшифровки";
    public static final String FOR_BRUTE_FORCE = "Brute Force";
    public static final String RETURN_PREV = "1. Вернуться назад";
    public static final String NO_ACTION = "Команда не найдена";
    public static final String ACTION_EXECUTED_OK = "Команда выполнилась успешно!\n";
}
